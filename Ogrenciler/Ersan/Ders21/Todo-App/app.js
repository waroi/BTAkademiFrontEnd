  ...
  request
    ...
    .then((data) =>
      data.map(function (data) {
        request
          .delete("http://localhost:3000/todo", data.id)
...
function deleteTodo(e) {
  ...
  if (e.target.className === "fa fa-remove") {
    ...
    deleteTodoFromStorage(e.target.id);
  }
}
...
function addTodoToUI(newTodo) {
  ...
  link.innerHTML = `<i class='fa fa-remove' id=${newTodo.id}></i>`;
  listItem.className = "list-group-item d-flex justify-content-between";
  listItem.appendChild(document.createTextNode(newTodo.todo));
